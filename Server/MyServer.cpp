...
void MyServer::OnConnect(TCPConnection& newConnection) {
	...
	*welcomeMessagePacket << std::string("Welcome!");
	...
	std::shared_ptr<Packet> newUserMessagePacket = std::make_shared<Packet>(PT_ChatMessage);
	*newUserMessagePacket << std::string("A new user connected");
...
void MyServer::OnDisconnect(TCPConnection& lostConnection, std::string reason) {
	...
	std::shared_ptr<Packet> connectionLostPacket = std::make_shared<Packet>(PT_ChatMessage);
	*connectionLostPacket << std::string("A user disconnected");
	...
	for (auto& connection : connections) {
		...
		connection.pm_outgoing.Append(connectionLostPacket); // sending message to all other users when user leave 
	}
